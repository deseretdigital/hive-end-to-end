import { start, report, logErrors, reportErrors, preview, promiseTimeout, retry, suite } from 'run';

export default async function run() {
    suite('Content updating, with Varnish');
    ...
    await start('http://varnish-secondary02.hive.pub/api/') // varnish
        ...
        .catch(logErrors)
    await start()
        .get(`content/beetdiggerfan/${body.site_content_id}`)
        .expectStatus(200)
        .expectKey('content_data.title')
        .expectValue('content_data.title', body.content_data.title)
        .end()
