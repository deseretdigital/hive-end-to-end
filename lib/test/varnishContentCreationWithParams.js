...
export default async function run() {
    ...
    await start(true)
        ...
        .catch(reportErrors)
    await start()
        .put(`content/${body.data.id}?action=publish&_import=true&_parseInlines=true`)
        .send({
            ...sendBody,
            content_data: {
    ...
    await start()
        .get(`content/beetdiggerfan/${body.data.site_content_id}?_loadrelated=true&_shortcodes=true`)
        .expectStatus(200)
        .expectKey('content_data.title')
        .expectValue('content_data.title', 'End-to-end test run (PUT update)')
        .end()
